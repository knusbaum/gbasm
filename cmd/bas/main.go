...
// smallest returns the smallest integer representation possible for i
func smallest(i uint64) interface{} {
	n := bits.Len64(i)
	if n <= 8 {
		return uint8(i)
	}
	...
	for scanner.Scan() {
		...
		for i := 1; i < len(parts); i++ {
			...
			if strings.HasPrefix(parts[i], "0x") {
				...
				args[i-1] = smallest(num)
				continue
			}
			if num, err := strconv.ParseUint(strings.TrimPrefix(parts[i], "0x"), 10, 64); err == nil {
				args[i-1] = smallest(num)
				continue
			}
			args[i-1] = parts[i]
