...
| I11 no use-after-discharge | ✓ | ✓ ptr-borrow / ✓ value-borrow (`#8` fixed) | ✓ ptr-borrow / ✓ value-borrow (`#8` fixed) | ✓ ptr-borrow / ✓ value-borrow (`#8` fixed) | ✓ | ✓ | · |
...
| I16 iface dispatch | ✓ (`iface_from_*`) | ✓ (`cov_iface_dispatch_field`, `#16` fixed) | · | · | ✓ (`iface_unknown_param`) | ✓ (`cov_iface_dispatch_return`) | ✓ dispatch + static-init (`#17` fixed, `cov_iface_global_static_init`) |
...
the user** (in §6, not closed): **#10** (pointee-construct (`new()`) stored borrow escapes
...
| I11 | owned-field **value** borrow (`b i64 := s.h`), then dispose struct | **silent** use-after-dispose — not tracked (binding-level `b := fd` IS) | **fixed** `(pending)` — `readProvenancePath` aliases an owned value-field/array-element read to the owner origin | `cov_owned_field_borrow_use_after_dispose_err`, `cov_nested_owned_value_use_after_dispose_err`, `cov_owned_array_elem_value_use_after_dispose_err` (green); live-owner read stays legal (`cov_owned_field_value_read_live`) |
| I11 | owned-**array** element value borrow (`b i64 := owned_arr[i]`), then dispose | same `#8` root, element position | **fixed** (above) — owned-array arm in `readAliasesOwnedStorage` | (with #8) |
...
| I11/I12 | a borrow **buried in an aggregate field** that leaves the frame, then dispose + read | **silent** use-after-dispose (false **negative**). Diagnosed precisely: tracked **in-frame** (direct construct + local struct copy, `cov_owned_field_borrow_in_struct_*`) but **dropped at every frame-leaving boundary** — normal call (P3), `new()`→pointee (P5/#10 headline), pointer write. *Not* `new()`-specific (a plain function drops it too); cause is that return-alias inference reconnects only **top-level pointer** returns (P1), with no interprocedural **field-buried** alias summary. **Big fix** — expanding the return-alias set's expressiveness touches compiler + assembler + object rep + grammar | **open #10 — deferred** | held: `cov_owned_field_borrow_escapes_{call,pointee_construct,pointee_store}_err` |
| — | owned field reached through a **pointer deref** (`pi.p` with `pi owned *mut inner`) reads as non-owned → `free requires an owned pointer, got *mut T` | **working as intended** — `owned *mut inner` owns the *pointer*, not the pointee, so the pointee is a view and its `owned` fields strip (borrowing-strips-ownership). To own the pointee's fields, use the stacked **`owned *owned mut inner`**: `free(pi.p)` then `free(pi)` both compile. (Earlier "can't destruct pointer-held structs with owned fields" was wrong — they can, via the stacked type.) Local owned struct `s owned inner` also frees `s.p` fine | **not a bug** (optional: case-B error could hint the stacked type; low priority) | A/B/C probes |
...
| I11/I12 | an **owned aggregate** returned from a call drops the alias tracking of its **borrowed** fields (`something(f) owned bar` returning `bar{y: f.x, z: owned}`); `free(src); *b.y` is **wrongly accepted** (use-after-free). Non-owned-return and in-frame versions are caught | **silent** use-after-free (false **negative**); caller's owned-struct-result path skips the `__callret` borrow recording the non-owned path uses | **open #18 — deferred** (found probing #10) | held: `cov_owned_aggregate_return_borrow_lost_err` |
| I7 | widening an i32 **parameter** to i64 (`return i64(x)`) fails in `bas`: `MOVSX ... Failed to find an instruction for Move with Sign-Extension`. Locals widen fine | **build failure** on valid code | **fixed** `(pending)` — `fixMovsx32To64` now retargets MOVSX→MOVSXD for a fixed-register dest too (was Ralloc-only; `operandBitWidth` helper) | `cov_cast_widen_param` (green) |
| I7 | narrowing a **global** i64 to i16 fails in `bas`: `MOV [y(size=16), g:16] Failed to find an instruction` — 16-bit move from a symbol-ref operand | **build failure** on valid code | **fixed** `(pending)` — `compileCast` stages a memory-backed/global source into a register before the `:N` partial narrow | `cov_cast_global_narrow` (green) |
...
| I16 | dispatch **directly** through an interface-typed struct field (`h.s.val()`) | was a deferred feature; **implemented** `(pending)` — type resolution (ast.go) resolves interface methods on an expression receiver; `compileInterfaceMethodCall` takes a receiver AST, computes the fat-pointer base address via the lvalue path, and keys the borrow-validity check on the receiver's flow path (`GetPathPointer`). Consuming (`owned`-receiver) methods still require a named binding | **fixed** | `cov_iface_dispatch_field` + `cov_iface_dispatch_field_borrow_live` (live ok) + `cov_iface_dispatch_field_borrow_after_dispose_err` (borrow-through-field invalidation enforced) |
| I16 | a global interface var with a **static** address-of initializer (`var gs speaker := &gx`) → `address-of initializer assigned to non-pointer type`; runtime assignment works | was a deferred feature; **implemented** `(pending)` — `encodeInterfaceAddressBytes` emits a 16-byte fat pointer with two relocations (data → target, reusing the pointer-data encoding; vtable → the vtable symbol). Vtable naming + `NeedVtable` factored into shared `interfaceVtableSymbol`, used by both the runtime and static-init paths | **fixed** | `cov_iface_global_static_init` (green) |
