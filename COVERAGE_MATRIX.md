...
| I15 | narrowing an **indexed array element** (`if (a[i] != nil) { *a[i] }`) does not narrow the element — must use `*a[i]?`. Field narrowing works (`cov_nullable_field_narrow`); element narrowing is unimplemented | minor **false rejection** (workaround: `?`) | **open — low priority** (likely intended: an index can change) | noted via `cov_nullable_array_literal` (uses `?`) |
